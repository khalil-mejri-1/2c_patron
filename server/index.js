...
// -------------------- F. Route لإضافة abonnement جديد (تم التعديل لإزالة الرفع المحلي) --------------------
// 💡 نستخدم الآن 'preuve_paiement_url' في الـ body بدلاً من رفع ملف
app.post('/api/abonnement', async (req, res) => {
  try {
    const { nom, mail, preuve_paiement_url } = req.body; // 💡 نستقبل الرابط مباشرةً من الـ body
...
// ❌ تم تعديل مسار DELETE لحذف الإشارة إلى حذف الملف المحلي
app.delete('/api/abonnement/:id', async (req, res) => {
  try {
    const abonnement = await Abonnement.findById(req.params.id);
