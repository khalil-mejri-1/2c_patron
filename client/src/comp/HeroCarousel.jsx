...
export default function HeroSection({ isLoggedIn = false, currentUserEmail = '' }) {
        ...
        const fetchProducts = async () => {
            try {
                ...
                const mappedProducts = sortedData.map((item) => ({
                    id: item._id,
                    price: item.prix,
                    name: getTranslated(item.nom),
                    url: item.image,
                    alt: getTranslated(item.nom),
    ...
    const handleViewDetails = (product) => {
        // Find matching product in main collection to get the real ID if possible
        fetch(`${BASE_URL}/api/products`)
            ...
            .then(allProducts => {
                ...
                const match = allProducts.find(p => {
                    ...
                    return pNameFr === searchName || pNameAr === searchName;
                });
                if (match) {
            ...
            .catch(() => navigate(`/product/${product.id}`));
    };
    ...
    const handleCardClick = (index) => {
        ...
        else if (offset === 0) handleViewDetails(products[index]);
    };
    ...
    return (
        <>
            <section className="modern-hero-section" dir={sectionDirection}>
                ...
                <div className="hero-content-block">
                    {isLoading ? (
                        <div className="hero-skeleton-content">
                            <div className="skeleton-line title shimmer" style={{ width: '70%', height: '60px', marginBottom: '20px' }}></div>
                            <div className="skeleton-line title shimmer" style={{ width: '40%', height: '60px', marginBottom: '30px' }}></div>
                            <div className="skeleton-line subline shimmer" style={{ width: '50%', height: '24px', marginBottom: '20px' }}></div>
                            <div className="skeleton-line intro shimmer" style={{ width: '90%', height: '18px', marginBottom: '10px' }}></div>
                            <div className="skeleton-line intro shimmer" style={{ width: '85%', height: '18px', marginBottom: '10px' }}></div>
                            <div className="skeleton-line intro shimmer" style={{ width: '60%', height: '18px', marginBottom: '40px' }}></div>
                            <div className="skeleton-button shimmer" style={{ width: '220px', height: '55px', borderRadius: '50px' }}></div>
                        </div>
                    ) : (
                        <>
                    {isAdmin && (
                    ...
                    {windowWidth > 1024 && (
                        <div className="hero-cta-wrapper small-cta1024">
                ...
                {windowWidth <= 1024 && (
                    <div className="hero-cta-wrapper large-cta1024 ">
                        <Link to="/magasin" className="hero-cta-button hero-cta-button-large">
                            {heroCtaTexts[currentLanguage] || texts.ctaButton}
                            {currentLanguage !== 'ar' && <FaArrowRight style={{ marginLeft: '10px' }} />}
                            {currentLanguage === 'ar' && <FaArrowRight style={{ transform: 'rotate(180deg)', marginRight: '10px' }} />}
                        </Link>
                        ...
                        {isLoading ? (
                            <>
                                ...
                                <div className="hero-card main-card skeleton-loading">
                                    <div className="hero-card-image-container skeleton-box shimmer"></div>
                                        ...
                                        {isMain && (
                                            <div className="hero-card-details-overlay">
                                                ...
                                                <div className="hero-card-footer">
                                                    ...
                                                    >
                                                        <FaHandPointer /> {texts.cartButton}
                                                    </button>
                                                </div>
                                            </div>
                                        )}
