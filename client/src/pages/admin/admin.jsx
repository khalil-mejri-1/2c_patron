...
export default function Admin() {
  ...
  const [isRightClickDisabled, setIsRightClickDisabled] = useState(false);
  ...
  useEffect(() => {
    ...
    const fetchSettings = async () => {
      try {
        ...
        setIsRightClickDisabled(value === true);
      } catch (err) {
        console.error("Error fetching right-click setting", err);
      }
    };
  ...
  const toggleRightClick = async () => {
    const newValue = !isRightClickDisabled;
    try {
      await fetch(`${BASE_URL}/api/settings/disable_right_click`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ value: newValue })
      });
      setIsRightClickDisabled(newValue);
      alert(newValue ? "Protection Clic Droit Activée" : "Protection Clic Droit Désactivée");
    } catch (err) {
      console.error("Error toggling right-click", err);
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '30px' }}>
              ...
              <div className="premium-card" style={{ padding: '30px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '15px', background: '#f8fafc', borderRadius: '12px', border: isRightClickDisabled ? '1px solid #D4AF37' : '1px solid transparent' }}>
                    <div style={{ fontSize: '0.9rem', color: '#1e293b' }}>
                      Protection Clic Droit
                      <div style={{ fontSize: '0.7rem', color: '#64748b' }}>{isRightClickDisabled ? 'Activé' : 'Désactivé'}</div>
                    </div>
                    <button 
                      onClick={toggleRightClick}
                      style={{
                        padding: '8px 16px',
                        background: isRightClickDisabled ? '#D4AF37' : '#e2e8f0',
                        color: isRightClickDisabled ? '#fff' : '#475569',
                        border: 'none',
                        borderRadius: '8px',
                        fontSize: '0.7rem',
                        fontWeight: 'bold',
                    ...
                    >
                      {isRightClickDisabled ? 'DÉSACTIVER' : 'ACTIVER'}
                    </button>
                  </div>
