...
const ImageCarousel = ({ images, direction, height = '320px' }) => {
    ...
    const goToNext = (e, isManual = false) => {
        if (e) e.stopPropagation();
        ...
        >
            ...
            >
                {images.map((url, index) => (
                    <div key={index} className="carousel-item" style={{ width: '100%', height: '100%', flexShrink: 0, background: '#fff' }}>
                        <img
                            src={url}
                            alt={`Slide ${index + 1}`}
                    ...
                    <div className="carousel-dots-indicators" style={{ position: 'absolute', bottom: '15px', width: '100%', textAlign: 'center', display: 'flex', justifyContent: 'center', gap: '6px', zIndex: 15 }}>
                        {images.map((_, index) => (
                            <span
                                ...
                                onClick={(e) => { e.stopPropagation(); setCurrentIndex(index); setIsAutoPlayEnabled(false); }}
                            ></span>
                        ))}
                    </div>
                </>
...
export default function ProductGrid() {
    ...
    const { appLanguage, languages, getTranslated } = useLanguage();
    ...
    // 🖼️ Carousel Image Filter UI States
    const [isSelectImagesModalOpen, setIsSelectImagesModalOpen] = useState(false);
    const [productForImageSelection, setProductForImageSelection] = useState(null);
    ...
    const fetchProducts = async () => {
        ...
        try {
            ...
            const mappedProducts = data.map(p => ({
                ...
                visibleCarouselImages: p.visibleCarouselImages || [], // 💡 Holds the selected images allowed for display in the grid carousel
                alt: getTranslated(p.nom),
    ...
    const getSafeText = (field) => {
        ...
        return `${field.fr || ''} ${field.ar || ''} ${field.en || ''}`.toLowerCase();
    };

    const filteredProducts = productsToFilter
        .filter(product => {
            const catString = typeof product.category === 'string' ? product.category : (product.category?.fr || product.category?.ar || '');
            const isCategoryMatch = selectedCategoryKey === 'Tous' || catString === selectedCategoryKey || product.category?.fr === selectedCategoryKey || product.category?.ar === selectedCategoryKey;

            if (!lowerSearchTerm) {
                return isCategoryMatch;
            } else {
                const isNameMatch = getSafeText(product.name).includes(lowerSearchTerm);
                const isSearchCategoryMatch = getSafeText(product.category).includes(lowerSearchTerm);
    ...
    // 6. Image Carousel Filter Modal (Admin Only)
    const CarouselImageSelectorModal = () => {
        if (!isSelectImagesModalOpen || !productForImageSelection) return null;

        const allImages = Array.from(new Set([productForImageSelection.url, ...(productForImageSelection.secondaryImages || [])].filter(Boolean)));

        const handleSave = async () => {
            try {
                const finalImages = selectedCarouselImages.filter(Boolean);
                const res = await fetch(`${BASE_URL}/api/products/${productForImageSelection.id}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ visibleCarouselImages: finalImages })
                });
                if (res.ok) {
                    setFetchedProducts(prev => prev.map(p => p.id === productForImageSelection.id ? { ...p, visibleCarouselImages: finalImages } : p));
                    showAlert('success', 'Succès', appLanguage === 'ar' ? 'تم الحفظ بنجاح' : 'Images mises à jour avec succès');
        ...
        const toggleImage = (url) => {
            if (selectedCarouselImages.includes(url)) {
                setSelectedCarouselImages(prev => prev.filter(img => img !== url));
            } else {
        ...
        return (
            <div className="premium-modal-backdrop" onClick={() => setIsSelectImagesModalOpen(false)} style={{ zIndex: 9999 }}>
                <div className="premium-modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: '800px', width: '90%' }}>
                    <button className="premium-modal-close-icon" onClick={() => setIsSelectImagesModalOpen(false)}><FaTimes /></button>
                    <h2 className="premium-modal-title" style={{ marginBottom: '20px', textAlign: 'center' }}>
                        {appLanguage === 'ar' ? 'تحديد صور الكاروسال' : 'Filtrer les images du carousel'}
                    </h2>

                    <p style={{ textAlign: 'center', marginBottom: '20px', color: '#64748b' }}>
                        {appLanguage === 'ar' ? 'اضغط على الصور التي تريد أن تظهر للزبائن داخل الكاروسال (البقية ستبقى مخفية).' : 'Sélectionnez les images à afficher dans le carousel (les autres seront masquées).'}
                    </p>

                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '15px', maxHeight: '50vh', overflowY: 'auto', padding: '10px' }}>
                        {allImages.map((imgUrl, idx) => {
                            const isSelected = selectedCarouselImages.includes(imgUrl);
                            return (
                                <div
                                    key={idx}
                                    ...
                                    style={{
                                        ...
                                        borderRadius: '12px',
                                        overflow: 'hidden',
                                        border: isSelected ? '4px solid #10b981' : '4px solid transparent',
                                ...
                                >
                                    <img src={imgUrl} alt={`img-${idx}`} style={{ width: '100%', height: '150px', objectFit: 'contain' }} />
                                    {isSelected && (
                                        <div style={{ position: 'absolute', top: '10px', right: '10px', background: '#10b981', color: '#fff', borderRadius: '50%', width: '25px', height: '25px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                            <FaCheck size={12} />
                                        </div>
                                    )}
                                </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', gap: '15px', marginTop: '30px' }}>
                        <button className="btn-cancel" onClick={() => setIsSelectImagesModalOpen(false)} style={{ padding: '12px 25px', background: '#f1f5f9', border: 'none', borderRadius: '12px', cursor: 'pointer', fontWeight: 'bold' }}>
                            {appLanguage === 'ar' ? 'إلغاء' : 'Annuler'}
                        </button>
                        <button className="btn-submit" onClick={handleSave} style={{ padding: '12px 25px', background: '#D4AF37', border: 'none', borderRadius: '12px', color: 'white', cursor: 'pointer', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <FaCheck /> {appLanguage === 'ar' ? 'حفظ' : 'Enregistrer'}
                        </button>
                    </div>
                </div>
            </div>
        ...
        <>
            ...
            <section className="product-grid-section" dir={direction}>
                ...
                >
                    ...
                    <div className="container shop_cont" style={{ position: 'relative', zIndex: 10, marginTop: '-100px' }}>
                                ...
                                activeOffers.map(offer => (
                                    ...
                                    >
                                        {isAdmin && (
                                            <div style={{ position: 'absolute', top: '10px', right: '10px', display: 'flex', gap: '8px', zIndex: 10 }}>
                                                <button
                                                    ...
                                                    onClick={(e) => { e.stopPropagation(); handleOpenEditOffer(offer); }}
                                                    style={{
                                                        background: '#fff',
                                                        color: '#D4AF37',
                                        ...
                                        <div className="offer-price-footer">
                                            ...
                                            <button className="offer-cta-btn">{appLanguage === 'ar' ? 'اطلب الآن ' : 'Commander '} <FaLongArrowAltRight /></button>
                                        </div>
                                    </div>
                                ))
                                ...
                                filteredProducts.map((product) => (
                                    <div key={product.id} className={`product-card ${isSelectionMode && selectedProductIds.includes(product.id) ? 'is-selected-for-offer' : ''}`} onClick={() => {
                                        ...
                                        <div className="product-image-wrapper">
                                            <ImageCarousel
                                                images={product.visibleCarouselImages?.length > 0 ? product.visibleCarouselImages : [product.url, ...(product.secondaryImages || [])].filter(Boolean)}
                                                direction={direction}
                                                height="100%"
                                            />
                                            {isAdmin && getSafeText(product.category).includes('offre') && (
                                                <button
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        setProductForImageSelection(product);
                                                        const allImages = Array.from(new Set([product.url, ...(product.secondaryImages || [])].filter(Boolean)));
                                                        setSelectedCarouselImages(product.visibleCarouselImages?.length > 0 ? product.visibleCarouselImages : allImages);
                                                        setIsSelectImagesModalOpen(true);
                                                    }}
                                                    style={{
                                                        position: 'absolute', top: '10px', left: '10px',
                                                        zIndex: 10, background: '#D4AF37', border: 'none', borderRadius: '10px', color: 'white', padding: '6px 12px', cursor: 'pointer', fontSize: '12px',
                                                        display: 'flex', alignItems: 'center', gap: '5px', boxShadow: '0 4px 10px rgba(0,0,0,0.2)'
                                                    }}
                                                    title="Chosir les images du carousel"
                                                >
                                                    <FaImages /> {appLanguage === 'ar' ? 'اختيار الصور' : 'Choisir Images'}
                                                </button>
                                            )}
                                            {isAdmin && (
                                ...
                                <div className="input-field-group">
                                    ...
                                    >
                                        ...
                                        {shopCategories.map(c => (
                                            <option key={c.key} value={c.key}>{getTranslated(c.name)}</option>
                                        ))}
                                    </select>
                                </div>
                                <div className="input-field-group">
